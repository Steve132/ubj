...
static inline void priv_ubjw_context_append(struct ubjw_context_t* ctx, uint8_t a)
{
	ctx->total_written += 1;
	ctx->write_cb(&a, 1, 1, ctx->userdata);
}

static inline void priv_ubjw_context_finish_container(struct ubjw_context_t* ctx, struct priv_ubjw_container_t* head)
{
...
static inline priv_ubjw_container_stack_push(struct ubjw_context_t* ctx, const struct priv_ubjw_container_t* cnt)
{
...
static inline struct priv_ubjw_container_t priv_ubjw_container_stack_pop(struct ubjw_context_t* ctx)
{
	return *ctx->head--;
}
...
static inline size_t priv_ubjw_context_write(struct ubjw_context_t* ctx, const uint8_t* data, size_t sz)
{
...
static inline void priv_ubjw_tag_public(struct ubjw_context_t* ctx, UBJW_TYPE tid)
{
...
static inline void priv_ubjw_write_raw_string(struct ubjw_context_t* ctx, const char* out)//TODO: possibly use a safe string
{
	size_t n = strlen(out);
...
static inline void priv_ubjw_write_raw_char(struct ubjw_context_t* ctx, char out)
{
...
static inline void priv_ubjw_write_raw_uint8(struct ubjw_context_t* ctx, uint8_t out)
{
...
static inline void priv_ubjw_write_raw_int8(struct ubjw_context_t* ctx, int8_t out)
{
	priv_ubjw_context_append(ctx, *(uint8_t*)&out);
}
...
static inline void priv_ubjw_write_raw_int16(struct ubjw_context_t* ctx, int16_t out)
{
	uint8_t buf[2];
...
static inline void priv_ubjw_write_raw_int32(struct ubjw_context_t* ctx, int32_t out)
{
	uint8_t buf[4];
...
static inline void priv_ubjw_write_raw_int64(struct ubjw_context_t* ctx, int64_t out)
{
	uint8_t buf[8];
...
static inline void priv_ubjw_write_raw_float32(struct ubjw_context_t* ctx, float out)
{
...
static inline void priv_ubjw_write_raw_float64(struct ubjw_context_t* ctx, double out)
{
...
void priv_ubjw_begin_container(struct priv_ubjw_container_t* cnt, struct ubjw_context_t* ctx, UBJW_TYPE typ, size_t count)
{
	cnt->flags=0;
...
static inline int priv_ubjw_type_size(UBJW_TYPE type)
{
	switch (type)
	{
	case UBJ_NULLTYPE:
...
static inline void priv_ubjw_write_byteswap(struct ubjw_context_t* ctx, const uint8_t* data, int sz, size_t count)
{
...
{
	int typesz = priv_ubjw_type_size(type);
	if (typesz < 0)
	{
